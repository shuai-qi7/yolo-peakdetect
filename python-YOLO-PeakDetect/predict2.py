...
class Detector(nn.Module):
        ...
        for i in range(anchor_box.size(0)):
            ...
            for anchor_box_ready in anchor_box_select:
                ...
                if anchor_box[i][0] > anchor_box_ready[0] and anchor_box[i][1] < anchor_box_ready[1]:
                    select_or_not = False
            if select_or_not:
                anchor_box_select.append(anchor_box[i])  # 选中当前预测框
        # 转换为张量
        anchor_box_select = torch.stack(anchor_box_select, dim=0)
        return anchor_box_select
    ...
    def __call__(self, sequence, target, max_value=1.0, k_x=1.0, k_y=1.0, width_max=20 * 255):
        ...
        # 调整预测振幅（应用y轴缩放因子）
        output_amplitude = output_amplitude / k_y
        # 按峰值位置排序（确保绘图时从左到右显示）
        ...
        output_area = output_amplitude * (output_left_stds + output_right_stds) * width_max / k_x

        # 打印预测的峰值位置和对应的面积（用于定量分析）
        print(output_locations.squeeze(-1))
        print(output_area.squeeze(-1))

        # -------------------------- 第一组可视化：原始序列与峰值边界 --------------------------
        # 创建2行1列的子图（用于展示原始序列和峰值边界）
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 4))
        # 处理原始序列：去除多余维度并应用y轴缩放
        sequence = sequence.squeeze(0).squeeze(0)
        sequence = sequence / k_y

        # 绘制预测的峰值边界和位置（红色标记）
        for i in range(len(output_sequences)):
            # 计算峰值的左右边界（均值±3倍标准差，覆盖99.7%的分布范围）
            x_start = output_locations[i] - 3 * output_left_stds[i]
            x_end = output_locations[i] + 3 * output_right_stds[i]

            # 绘制边界标记（竖线）
            ax2.scatter(x_start, (-1.5 - 0.05 * i), color='red', marker='|', s=20)
            ax2.scatter(x_end, (-1.5 - 0.05 * i), color='red', marker='|', s=20)

            # 绘制水平箭头表示峰值的宽度范围
            ax2.annotate(
                '', xy=(x_end, (-1.5 - 0.05 * i)), xytext=(x_start, (-1.5 - 0.05 * i)),
                arrowprops=dict(facecolor='red', shrink=0.05, width=0.1, headwidth=1, headlength=0.8, color='red')
            )
            # 绘制指向峰值中心的箭头
            ax2.annotate(
                '', xy=(x_start, (-1.5 - 0.05 * i)), xytext=((x_start + x_end) / 2, (-1.5 - 0.05 * i)),
                arrowprops=dict(facecolor='red', shrink=0.05, width=0.1, headwidth=1, headlength=0.8, color='red')
            )
            # 标记峰值位置（红色三角形）
            ax2.scatter(output_locations[i], -1.25, color='red', marker='^', s=100, label='Predicted Peak')

        # 绘制真实目标的峰值边界和位置（蓝色标记）
        for i in range(len(target_locations)):
            x_start = target_locations[i] - 3 * target_left_stds[i]
            x_end = target_locations[i] + 3 * target_right_stds[i]

            # 绘制边界标记（竖线）
            ax2.scatter(x_start, (-0.5 - 0.05 * i), color='blue', marker='|', s=20)
            ax2.scatter(x_end, (-0.5 - 0.05 * i), color='blue', marker='|', s=20)

            # 绘制水平箭头表示真实峰值的宽度范围
            ax2.annotate(
                '', xy=(x_end, (-0.5 - 0.05 * i)), xytext=(x_start, (-0.5 - 0.05 * i)),
                arrowprops=dict(facecolor='blue', shrink=0.05, width=0.1, headwidth=1, headlength=0.8, color='blue')
            )
            # 绘制指向真实峰值中心的箭头
            ax2.annotate(
                '', xy=(x_start, (-0.5 - 0.05 * i)), xytext=((x_start + x_end) / 2, (-0.5 - 0.05 * i)),
                arrowprops=dict(facecolor='blue', shrink=0.05, width=0.1, headwidth=1, headlength=0.8, color='blue')
            )
            # 标记真实峰值位置（蓝色三角形）
            ax2.scatter(target_locations[i], -0.25, color='blue', marker='^', s=100, label='True Peak')

        # 绘制原始序列（黑色曲线）
        ax1.plot(sequence, color='black')
        # 设置坐标轴标签
        ax1.set_ylabel('Amplitude ')  # ax1的y轴标签（振幅）
        ax2.set_ylabel('Amplitude ')  # ax2的y轴标签
        # 设置坐标轴范围（确保波形完整显示）
        ax1.set_ylim(0, 1.2 * max(sequence))  # y轴范围为振幅最大值的1.2倍
        ax2.set_ylim(-2, 0)  # 下方子图范围（用于显示峰值标记）
        ax1.set_xlim(0, 1024)  # x轴范围（假设数据长度为1024）
        ax2.set_xlim(0, 1024)
        # 自动调整布局，避免标签重叠
        plt.tight_layout()
        plt.show()  # 显示第一组图形
        ...
        # -------------------------- 第二组可视化：原始序列与预测高斯峰 --------------------------
        # 创建2行1列的子图（用于展示原始序列和预测的高斯峰）
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(16, 4))

        # 在ax2中绘制每个预测的高斯峰（不同颜色区分）
        for i in range(len(output_sequences)):
        ...
        # 设置坐标轴标签
        ax1.set_ylabel('Amplitude ')  # ax1的y轴标签
        ax2.set_ylabel('Amplitude ')  # ax2的y轴标签
        # 设置坐标轴范围
...
if __name__ == "__main__":
    """主函数：测试检测器的功能，包括从数据集加载数据和从图像转换数据两种模式"""
    with warnings.catch_warnings():
        warnings.simplefilter('ignore')  # 忽略警告信息

        # 模式1：从数据集加载预测数据并检测
        for i in range(5):
            data = DataSet(num_samples=10, data_type='predict')  # 加载预测数据集
            sequence, target, _ = data[i]  # 获取一个样本
            sequence = sequence.unsqueeze(0)  # 增加批次维度
            target = target.unsqueeze(0)
            detector = Detector()  # 实例化检测器
            detector(sequence, target)  # 执行检测
